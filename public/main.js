  ...
  fetch("http://localhost:3000/forecast?city=" + cityID)
    ...
    .then(function (wedata) {
      ...
      for (let i = 0, j = 0; i < 24, j < 3; i += 8, j++) {
        ...
        document.querySelectorAll(".temp")[j].textContent =
          Math.round(parseFloat(tempNow - 273.15)) + "  ℃";
        document.querySelectorAll(".imgico")[j].src =
          imgs[wedata.list[i].weather[0].main];
        document.querySelectorAll(".desc")[j].textContent =
          wedata.list[i].weather[0].main;
      }
    })
    .catch(function (err) {
      console.log(err.message, "err mess");
